...
#[derive(Debug, Clone)]
pub struct MountrootV1 {
    mountroot: Vec<Mountroot>,
}
...
impl MountrootV1{
    pub fn new() -> MountrootV1 {
        MountrootV1 {
            mountroot: get_mount_roots(),
        }
    }

    pub fn get_subsystems_mountroots(self) -> HashMap<String, String> {
        let mount_roots = self.mountroot;
        let mut subs = HashMap::new();

        if let Some(root) = get_mount_root(&mount_roots, Controllers::BlkIo) {
            subs.insert(Controllers::BlkIo.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Mem) {
            subs.insert(Controllers::Mem.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Pids) {
            subs.insert(Controllers::Pids.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::CpuSet) {
            subs.insert(Controllers::CpuSet.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::CpuAcct) {
            subs.insert(Controllers::CpuAcct.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Cpu) {
            subs.insert(Controllers::Cpu.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Devices) {
            subs.insert(Controllers::Devices.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Freezer) {
            subs.insert(Controllers::Freezer.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::NetCls) {
            subs.insert(Controllers::NetCls.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::PerfEvent) {
            subs.insert(Controllers::PerfEvent.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::NetPrio) {
            subs.insert(Controllers::NetPrio.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::HugeTlb) {
            subs.insert(Controllers::HugeTlb.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Rdma) {
            subs.insert(Controllers::Rdma.to_string(), root.into_os_string().into_string().unwrap());
        }
        if let Some(root) = get_mount_root(&mount_roots, Controllers::Systemd) {
            subs.insert(Controllers::Systemd.to_string(), root.into_os_string().into_string().unwrap());
        }
...
pub fn get_mount_roots() -> Vec<Mountroot> {
    // get mount root
    match File::open("/proc/self/mountinfo") {
        Ok(mut file) => mountroot_file(&mut file),
        Err(_) => vec![],
    }

}

fn mountroot_file(file: &mut File) -> Vec<Mountroot> {
    let mut r = Vec::new();
    for line in BufReader::new(file).lines() {
        match line {
            Ok(line) => {
                if let Some(mi) = parse_mountroot_for_line(&line) {
                    if mi.fs_type.0 == "cgroup" {
                        r.push(mi);
                    }
                }
...
fn parse_mountroot_for_line(line: &str) -> Option<Mountroot> {
    let s_values: Vec<_> = line.split(" - ").collect();
    if s_values.len() != 2 {
        return None;
    }
...
pub fn get_mount_root(mount_roots: &Vec<Mountroot>, controller: Controllers) -> Option<PathBuf> {
    mount_roots.iter().find_map(|m| {
        if m.fs_type.0 == "cgroup" && m.super_opts.contains(&controller.to_string()) {
            return Some(m.mount_root.clone());
        }
        None
    })
}
