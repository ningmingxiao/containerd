...
pub fn existing_path(paths: HashMap<String, String>) -> Result<HashMap<String, String>> {
    let mount_roots_v1  = crate::hierarchies::MountrootV1::new();
    let mut mount_roots_subsystems_map = mount_roots_v1.get_subsystems_mountroots();
    
    for (k,v) in paths {
        let mr =  mount_roots_subsystems_map.get(&k).unwrap();
        
        let des_path = get_cgroup_destination((*mr).clone(), v);
        mount_roots_subsystems_map.insert(k, des_path);
    }
