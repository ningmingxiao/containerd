...
fn register_memory_event(
    ...
    thread::spawn(move || {
        loop {
            let mut buf = [0; 8];
            // when the read function is interrupted by some signal,
            // error of ErrorKind::Interrupted may be returned.
            // This is non-fatal and the read operation should be retried.
            if let Err(e) = eventfd_file.read(&mut buf) {
                match e.kind() {
