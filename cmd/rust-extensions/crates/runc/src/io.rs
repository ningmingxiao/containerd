...
impl BinaryIO {
    pub fn new(id: &str, u: &url::Url, ns: &str) -> std::io::Result<Self> {
        Ok(Self {
            cmd: BinaryIO::binary_cmd(id, u, ns)?,
            stdout: Some(Pipe::new()?),
            stderr: Some(Pipe::new()?),
        })
    }

    pub unsafe fn create_binary_io(&mut self) -> Result<()> {
        let mut sync_pipe = Pipe::new()?;
        let wr_fd = sync_pipe.wr.as_raw_fd();
        let stdout_rd_fd = self.stdout.as_ref().unwrap().rd.as_raw_fd();
        let stderr_rd_fd = self.stderr.as_ref().unwrap().rd.as_raw_fd();
        self.cmd.pre_exec(move || {
            if libc::dup2(stdout_rd_fd, 3) < 0 {
                debug!("syscall dup2 stdout_rd err: {}", errno());
                return Err(std::io::Error::new(ErrorKind::Other, "libc dup2 err"))
            }
            if libc::dup2(stderr_rd_fd, 4) < 0 {
                debug!("syscall dup2 stderr_rd err: {}", errno());
                return Err(std::io::Error::new(ErrorKind::Other, "libc dup2 err"))
            }
            if libc::dup2(wr_fd, 5) < 0 {
                debug!("syscall dup2 rw err: {}", errno());
                return Err(std::io::Error::new(ErrorKind::Other, "libc dup2 err"))
            }
            Ok(())
        });
        self.cmd.spawn().expect("binary process failed to start").try_wait()?;
        // Close write pipe in shim.
        nix::unistd::close(sync_pipe.wr.as_raw_fd()).unwrap_or_else(|e| debug!("close write pipe: {}", e));
        // Wait for the logging binary to be ready.
        let buffer = &mut [0; 1];
        let _ = sync_pipe.rd.read(buffer)?;
        // Read pipe is useless, free it here.
        nix::unistd::close(sync_pipe.rd.as_raw_fd()).unwrap_or_else(|e| debug!("close read pipe: {}", e));
        debug!("binary cmd start success");
        Ok(())
    }

    pub fn binary_cmd(id: &str, u: &url::Url, ns: &str) ->Result<Command> {
        let mut cmd = Command::new(u.path());
        let mut args: Vec<String> = Vec::new();
        for query in u.query_pairs() {
            if !query.0.is_empty() {
                args.push(query.0.parse().unwrap());
            }
            if !query.1.is_empty() {
                args.push(query.1.parse().unwrap());
            }
        }
        cmd.env("CONTAINER_ID", id).env("CONTAINER_NAMESPACE", ns).args(&args);
        Ok(cmd)
    }
}
