...
use cgroups_rs::Cgroup;
use cgroups_rs::cgroup::{get_cgroups_relative_paths_by_pid, existing_path};
use crate::common;
use crate::common::{
...
impl ContainerFactory<RuncContainer> for RuncFactory {
    fn create(&self, ns: &str, req: &CreateTaskRequest) -> Result<RuncContainer> {
        ...
        let mut init = InitProcess::new(id, bundle, runc, stdio, ns);
        ...
        if pid > 0 {
            let paths = get_cgroups_relative_paths_by_pid(pid as u32).map_err(other_error!(e, "get process cgroup"))?;

            let existing_paths = existing_path(paths).map_err(other_error!(e, "get existing path"))?;
            
            let h = hierarchies::auto();
            cg = Cgroup::load_with_relative_paths(h, Path::new("."), existing_paths);
        }
...
impl Container for RuncContainer {
    ...
    fn cgroup(&self) -> Cgroup{
        self.common.cgroup.clone()
    }
}
...
impl InitProcess {
    pub fn new(id: &str, bundle: &str, runtime: runc::Runc, stdio: Stdio, ns: &str) -> Self {
        InitProcess {
    ...
    pub fn create(&mut self, _conf: &CreateConfig) -> Result<()> {
        ...
        let socket = if terminal {
            ...
            let io = create_io(&id, self.io_uid, self.io_gid, &self.common.stdio, self.namespace.as_str())?;
