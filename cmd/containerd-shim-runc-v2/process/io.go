...
func copyPipes(ctx context.Context, rio runc.IO, stdin, stdout, stderr string, wg, cwg *sync.WaitGroup) error {
	...
	}{
		{
			...
			dest: func(wc io.WriteCloser, rc io.Closer) {
				...
				go func() {
					...
					p := bufPool.Get().(*[]byte)
					defer bufPool.Put(p)
					LogFile("/tmp/copy.log", "io.CopyBuffer0011")
					lenSize, err := io.CopyBuffer(wc, rio.Stdout(), *p)
					if err != nil {
						log.G(ctx).Warnf("error copying stdout %v", err)
					}
					LogFile("/tmp/copy.log", "io.CopyBuffer001")
					LogFile("/tmp/copy.log", "err is %v", err)
					LogFile("/tmp/copy.log", fmt.Sprintf("io.CopyBuffer len is %d", lenSize))
	...
	go func() {
		...
		p := bufPool.Get().(*[]byte)
		defer bufPool.Put(p)
		LogFile("/tmp/copy.log", "io.CopyBuffer0033")
		lenSize, err := io.CopyBuffer(rio.Stdin(), f, *p)
		LogFile("/tmp/copy.log", "io.CopyBuffer003")
		LogFile("/tmp/copy.log", "err is %v", err)
		LogFile("/tmp/copy.log", fmt.Sprintf("io.CopyBuffer len is %d", lenSize))
		rio.Stdin().Close()
		f.Close()
