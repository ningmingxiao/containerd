...
func NewTaskService(ctx context.Context, publisher shim.Publisher, sd shutdown.Service) (taskAPI.TTRPCTaskService, error) {
	...
	s := &service{
		...
		lastExecEventSent:    make(map[*runc.Container]bool),
		exitSubscribers:      make(map[*map[int][]runcC.Exit]struct{}),
		containerInitProcess: make(map[*runc.Container]initProcess),
		execRunned:           make(map[*runc.Container]bool),
	}
	go s.processExits()
...
type service struct {
	...
	lastExecEventSent    map[*runc.Container]bool
	containerInitProcess map[*runc.Container]initProcess
	// check  exec has been runned
	execRunned map[*runc.Container]bool
...
func (s *service) handleProcessExit(e runcC.Exit, c *runc.Container, p process.Process) {
	if ip, init := p.(*process.Init); init {
		...
		go func() {
			s.execsMu.Lock()
			execRuned := s.execRunned[c]
			s.execsMu.Unlock()
			if execRuned {
				times := 1
				maxDuration := 10
				for !s.lastExecEventSent[c] {
					if times == maxDuration {
						s.execsMu.Lock()
						pids, ok := s.runningExecs[c]
						s.execsMu.Unlock()
						if ok {
							//print exec process pid that can't be killed.
							for _, pid := range pids {
								log.G(s.context).Warnf("exec process %d is still running", pid)
							}
						}
					}
		...
		if len(s.runningExecs[c]) == 0 {
			// last exec event is sent
			s.lastExecEventSent[c] = true
		}
		s.execsMu.Unlock()
	}
