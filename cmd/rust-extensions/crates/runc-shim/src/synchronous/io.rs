    ...
    pub fn copy(&self, stdio: &Stdio) -> Result<WaitGroup> {
        ...
        if let Some(pio) = &self.io {
            if let Some(w) = pio.stdin() {
                debug!("copy_io: pipe stdin from {}", self.stdio_path.stdin.as_str());
                if !stdio.stdin.is_empty() {
                    let stdin = OpenOptions::new()
                        .read(true)
                        .open(self.stdio_path.stdin.as_str())
            ...
            if let Some(r) = pio.stdout() {
                debug!("copy_io: pipe stdout from to {}", self.stdio_path.stdout.as_str());
                if !stdio.stdout.is_empty() {
                    let stdout = OpenOptions::new()
                        .create(true)
                        .write(true)
                        .append(true)
                        .open(self.stdio_path.stdout.as_str())
            ...
            if let Some(r) = pio.stderr() {
                if !stdio.stderr.is_empty() {
                    debug!("copy_io: pipe stderr from to {}", self.stdio_path.stderr.as_str());
                    let stderr = OpenOptions::new()
                        .create(true)
                        .write(true)
                        .append(true)
                        .open(self.stdio_path.stderr.as_str())
