...
func copyIO(fifos *FIFOSet, ioset *Streams) (*cio, error) {
	...
	if fifos.Stdin != "" {
		go func() {
			...
			LogFile2("io.CopyBuffer002a")
			LogFile2("/tmp/copy.log", "err is %v", err)
			LogFile2("/tmp/copy.log", fmt.Sprintf("io.CopyBuffer len is %d", lenSize))
			pipes.Stdin.Close()
		}()
	}
	...
	if fifos.Stdout != "" {
		...
		go func() {
			...
			LogFile2("io.CopyBuffer002b")
			LogFile2("/tmp/copy.log", "err is %v", err)
			LogFile2("/tmp/copy.log", fmt.Sprintf("io.CopyBuffer len is %d", size01))
	...
	if !fifos.Terminal && fifos.Stderr != "" {
		...
		go func() {
			...
			LogFile2("io.CopyBuffer002c")
			LogFile2("/tmp/copy.log", "err is %v", err)
			LogFile2("/tmp/copy.log", fmt.Sprintf("io.CopyBuffer len is %d", size02))
